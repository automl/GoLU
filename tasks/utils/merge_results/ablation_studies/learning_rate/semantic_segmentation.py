...
architectures_lrs = {
    "deeplabv3_resnet50":  [0.1, 0.04, 0.03, 0.02, 0.01, 0.001, 0.0001],
}
...
default_lrs = {
    'deeplabv3_resnet50': 0.02,
}

best_lrs = {
    'deeplabv3_resnet50': 0.01,
}
...
for arch, (df_mean, df_stderr) in architecture_dfs.items():
    ...
    ax = sns.heatmap(
        ...
        fmt=".4f",           # format floats to 2 decimal places
        cmap="coolwarm",
        cbar=True
    )
    ...
    for row in annotation_df.index:
        for col in annotation_df.columns:
            ...
            if pd.isna(m) or pd.isna(s):
                ...
                annotation_df.loc[row, col] = f"{m:.4f}\n±{s:.3f}"
