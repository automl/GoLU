...
from tasks.utils.utils import load_json, save_results
...
group.add_argument('--activation', default='', type=str, metavar='ACTIVATION',
...
def main():
    ...
    seed = int(args.seed)
    args.output = f'{args.output}/{dataset}/{model_name}/{activation}'
    opt_trace_path = f'{args.output}/opt_trace_{seed}.json'
    results_path = f'{args.output}/results_{seed}.json'
    plots_path = f'{args.output}/plot_{seed}.png'
    ...
    if utils.is_primary(args):
        _logger.info(f'Dataset - {dataset} | Model - {model_name} | Activation - {activation} | Seed - {seed}')
    ...
    if os.path.exists(opt_trace_path):
        ...
        if utils.is_primary(args):
            _logger.info(f'Loaded optimization trace from path - {opt_trace_path}')
    ...
    if 'efficient' in model_name:
        model = replace_activation_by_torch_module(model, nn.SiLU, activation)
    ...
    if utils.is_primary(args):
        ...
        output_dir = utils.get_outdir(args.output if args.output else './output/train', str(seed))
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            opt_trace['train_loss'].append(train_metrics['loss'])
            opt_trace['train_top1_acc'].append(train_metrics['top1'])
            ...
            if loader_eval is not None:
                ...
                opt_trace['test_loss'].append(eval_metrics['loss'])
                opt_trace['test_top1_acc'].append(eval_metrics['top1'])
            ...
            if utils.is_primary(args):
                ...
                logging.info(
                    f"{datetime.now()} Epoch [{epoch + 1}/{num_epochs}] - \
                        Train Loss - {opt_trace['train_loss'][-1]:.4f} | \
                        Train Top-1 Acc - {opt_trace['train_top1_acc'][-1]:.4f} | \
                        Test Loss - {opt_trace['test_loss'][-1]:.4f} | \
                        Test Top-1 Acc - {opt_trace['test_top1_acc'][-1]:.4f} | \
                        Train Time - {train_time:.4f} | Inference Time - {inference_time:.4f}")
